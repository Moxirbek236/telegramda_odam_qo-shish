...
Environment overrides for `add_mem.py`:

```env
SOURCE_GROUP_ID=-1001937901847
TARGET_GROUP_ID=-1002000540919
MAX_SUCCESSFUL_ADDS=100
INVITE_DELAY_SECONDS=60
FLOODWAIT_HEARTBEAT_SECONDS=60
AUTO_RAISE_DELAY_ON_FLOODWAIT=true
SAFE_DELAY_AFTER_FLOODWAIT=60
RUN_FOREVER=true
IDLE_SLEEP_SECONDS=300
RETRY_ON_FATAL_SECONDS=30
...
Use the included `render.yaml` to deploy as a **Worker** service.

1. Push this repo to GitHub.
2. In Render, create a new Blueprint deployment from the repo.
3. Use the generated worker service (`plan: starter` in `render.yaml`).
4. Set these required environment variables in Render:
   * `TELEGRAM_API_ID`
   * `TELEGRAM_API_HASH`
   * `TELEGRAM_SESSION_STRING`

Generate `TELEGRAM_SESSION_STRING` locally:

```bash
python generate_session.py
```

Important: Render **Free Web Service** spins down on idle. If you need no sleep behavior, use a paid plan (for example, the worker `starter` plan in `render.yaml`).
